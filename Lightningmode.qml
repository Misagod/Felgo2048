...
SceneBase{
    ...
    Component.onCompleted: {
        ...
        //setInterval(freshWithTime(), 2000)
    }
    Row{
        width: gridWidth
        height: 50
    ...
    Rectangle {
        z: -1
        anchors.fill: lightningMode.gameWindowAnchorItem
    ...
    function freshWithTime()
    {
        ...
        if(emptyCells.length === 0)
        {
            if(checkIfFail())
            {
                ...
                window.previousScene = "lightningmode"
                isRunning = 0
            }
        }
    }

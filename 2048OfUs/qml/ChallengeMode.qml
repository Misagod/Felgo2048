...
SceneBase{
    ...
    property int isWin: 0
    property int presentTask
    ...
    Rectangle {
        ...
        anchors.fill: challengeMode.gameWindowAnchorItem
    ...
    function moveLeft() {
        ...
        if (isMoved) {
            ...
            createNewTile()
            task.leftSteps--;
            if(task.leftSteps === 0 && isWin === 0)
            {
                window.state = "gameover"
                window.previousScene = "challengemode"
                newGame()
            }
...
//        score = 0
    ...
    function setTask(level)
    {
        ...
        if(level === 0)
        {
            ...
            task.task1 = 4
            task.task1unfinished = 1

            task.task2 = 8
            task.task2unfinished = 1

            task.task3 = 16
            task.task3unfinished = 1

            task.task1finished = 0
            task.task2finished = 0
            task.task3finished = 0
    ...
    function updateTasks(value)
    {
        ...
        if(value === task.task1 && (task.task1finished !== task.task1unfinished))
        {
            task.task1finished++
        }else if(value === task.task2 && (task.task2finished !== task.task2unfinished))
        {
            task.task2finished++
        }else if(value === task.task3 && (task.task3finished !== task.task3unfinished))
        {
            task.task3finished++
        }
        if(task.task1finished === task.task1unfinished && task.task2finished === task.task2unfinished && task.task3finished === task.task3unfinished)
        {
            isWin = 1
            window.state = "challengecomplete"
            newGame()
//            task.task1finished = 0
//            task.task2finished = 0
//            task.task3finished = 0
    ...
    //        console.log(value)
    //        var i;
    //        for(i = 0; i < task.modelNum; i++)
    //        {
    //            if(Math.pow(2, value) === task.tasks[i*2])
    //            {
    //                task.taskFinished[i]++
    //                console.log(task.taskFinished[i])
    //            }
    //        }

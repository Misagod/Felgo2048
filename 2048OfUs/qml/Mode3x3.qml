...
SceneBase{
    ...
    property int gridSizeGame: 3 // game grid size in tiles
    ...
    Rectangle {
        z: -1
        anchors.fill: mode3x3.gameWindowAnchorItem
        ...
        if (isMoved) {
            ...
            if(emptyCells.length === 0)
            {
                if(checkIfFail())
                {
                    ...
                    scoreScene.score = mode3x3.score
                    window.previousScene = "mode3x3"
                }
            }
        }
    }
